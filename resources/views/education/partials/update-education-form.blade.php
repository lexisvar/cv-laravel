<section>
  ...
  <form method="post" action="{{ route('education.update', $education->id) }}" class="mt-6 space-y-6">
      ...
      <div>
        ...
        <x-text-input id="start_date" name="start_date" type="date" class="mt-1 block w-full" :value="old('start_date', $education->start_date)" required autocomplete="start_date" />
        <x-input-error class="mt-2" :messages="$errors->get('start_date')" />
      </div>

      <div>
        <x-input-label for="finish_date" :value="__('Finish Date')" />
        <x-text-input id="finish_date" name="finish_date" type="date" class="mt-1 block w-full" :value="old('finish_date', $education->finish_date)" required autocomplete="finish_date" />
